...
- Here, the ATT identification relies on the Parallel trend assumption. 
...
Consider T periods of time, and that treatment happen at period G

We could use the same equation as before, and estimate ATT for any period of time (relative to G)

$$ATT(T) = E(Y_{i,T} - Y_{i,G-1}|D_i=1) - E(Y_{i,T} - Y_{i,G-1}|D_i=0)
$$

- We could use "post treatment" data to estimate the ATTs. 

- and use "pre treatment" data as a proxy test for PTA. 

- If PTA holds, then $E(ATT(T)|T<G) = 0$
...
A similar story could be told about other types of variables.

In general, it is possible that Parallel trends hold for some other monotonic transformation of the outcome variable.
...
$$E_1\Big[Y_{i,1}(0))\Big] - E_1\Big[g(Y_{i,0}(0))\Big] = E_0\Big[g(Y_{i,1})\Big] - E_0\Big[g(Y_{i,0})\Big]
$$
...
- For the practitioner, the extended Nonlinear DID simply requires choosing a functional form that better fits the data.
  - Poisson, logit, fractional regression, Linear model, etc
...
$$y_{i,t} = \beta_0 + \beta_1 D + \beta_2 T + \theta (D \times T) + T \times X\gamma_T + D \times X\gamma_d + D \times T (X-\bar X )\lambda + \epsilon_{i,t}$$
...
- This model has been shown to be problematic, because of How OLS estimates the $\theta^{fe}$ parameter.
  ...
  - Some designs use early treated units as controls for late treated units, which is a violation of the parallel trends assumption. (effectively receiving negative weights) @goodmanbacon2021, @dechaisemartin2020 and @borusyak2023revisiting 
...
## Implementing Nonlinear DID

- The `jwdid` package implements the approach proposed by @wooldridge2022 and @wooldrige2023, and allows for the estimation of the ATTs using different functional forms.
