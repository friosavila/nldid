...
- The canonical DiD, a 2x2 design, simply compares means (or conditional means) of the outcome variable (before after x treated non-treated) to identify treatment effects. 
  - Thus it can be used even if outcome is a limited dependent variable (binary, count, etc) 
    - Parallel to the linear regression case
    - Assumes Linear PTA
...
- This insight can be extended to nonlinear cases.
...
- However, if we "move", from the 2x2 design, 
  - it may be possible to test if PTA hold Before treatment.
  - Or use a DDD design to indirectly test for PTA.
...
## PTA may hold for $G^{-1}(\bar Y)$

A similar story could be told about other types of transformations.

In general, it is possible that PTA hold for some monotonic transformation of the outcome variable $G^{-1}()$.
...
- `jwdid` - Command Name. In charge of getting all interactions -right-
  - `depvar` `[indepvar]` - Declare the dependent variables. Independent variables are optional, but should be time fixed.
  - An example with RC [here](https://friosavila.github.io/app_metrics/app_metrics8.html)
...
Visit my website [website](https://friosavila.github.io/)
